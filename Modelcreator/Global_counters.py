# -*- coding: utf-8 -*-
"""
Created on Mon Dec 01 15:11:56 2014

@author: Acer
"""

class Global_counters:

        
    number_of_buildings = 0
    number_of_connections = 0
    number_of_greywatertanks = 0     
    number_of_raintanks = 0
    number_of_collectors = 0
    number_of_distributors = 0
    number_of_stormwaterpipes = 0
    number_of_sewers = 0
    number_of_potablwaterreservoirs = 0
    number_of_catchments = 0
    number_of_demandmodels = 0
    number_of_stormwaterreservoirs = 0
    number_of_filereaders = 0
    number_of_patternimplementers = 0
    number_of_fileouts = 0
    number_of_gardenwateringmodules = 0
    number_of_greywaterreservoirs = 0
    number_of_collectors_ports_list = []
    number_of_distributors_ports_list = []
    numbers_names_of_fileouts_list = []
        

    


