...
class Raintanksetup:
    ...
    def __init__(self, numberofRaintanks, starting_value_i, **Raintankattributes ):
        ...
        for i in range(numberofRaintanks+starting_value_i)[starting_value_i:]:
            exec 'self.Raintankattributelist.append({"Raintank_'+str(i)+'" : Raintankattributes})'
    ...
    def Setandwrite_attributes(self,numberofRaintanks, starting_value_i, attributevector):
        for i in range(numberofRaintanks+starting_value_i)[starting_value_i:]:
            self.Raintankattributelist[i][str('Raintank_'+str(i))]["Storage_Volume"] = attributevector[i][0]
    
        for i in range(numberofRaintanks+starting_value_i)[starting_value_i:]:
            exec '''self.line1='\\t\\t\\t<node id="Raintank_'+str(i)+'" class="Raintank"/> \\n' '''
            exec '''self.line2='\\t\\t\\t\\t<parameter name="Storage_Volume_[m^3]" type="double" value="'+str(self.Raintankattributelist[i][str('Raintank_'+str(i))]["Storage_Volume"])+'"/> \\n ' '''
