...
class Distributorsetup:
    ...
    def __init__(self, numberofDistributors, starting_value_i, **Distributorattributes ):
        ...
        for i in range(numberofDistributors+starting_value_i)[starting_value_i:]:
            exec 'self.Distributorattributelist.append({"Distributor_'+str(i)+'" : Distributorattributes})'
    ...
    def Setandwrite_attributes(self,numberofDistributors, starting_value_i, attributevector):
        for i in range(numberofDistributors+starting_value_i)[starting_value_i:]:
            self.Distributorattributelist[i][str('Distributor_'+str(i))]["Number_of_Outports"] = attributevector[i][0]
    
        for i in range(numberofDistributors+starting_value_i)[starting_value_i:]:
            exec '''self.line1='\\t\\t\\t<node id="Distributor_'+str(i)+'" class="Distributor"/> \\n' '''
            exec '''self.line2='\\t\\t\\t\\t<parameter name="Number_of_Outports" type="int" value="'+str(self.Distributorattributelist[i][str('Distributor_'+str(i))]["Number_of_Outports"])+'"/> \\n ' '''
