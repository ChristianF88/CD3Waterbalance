...
class Demandmodelsetup:
    ...
    def Setandwrite_attributes(self,numberofDemand_Models, starting_value_i, attributevector):
        for i in range(numberofDemand_Models+starting_value_i)[starting_value_i:]:
            ...
            self.Demand_Modelattributelist[i-starting_value_i][str('Demand_Model_'+str(i))]["Select_Model_(Simple_Model_or_Stochastic_Model)"] = attributevector[i-starting_value_i][2]
        ...
        for i in range(numberofDemand_Models+starting_value_i)[starting_value_i:]:
            ...
            exec '''self.line4='\\t\\t\\t\\t<parameter name="Select_Model_(Simple_Model_or_Stochastic_Model)" type="string" value="'+str(self.Demand_Modelattributelist[i-starting_value_i][str('Demand_Model_'+str(i))]["Select_Model_(Simple_Model_or_Stochastic_Model)"])+'"/> \\n ' '''
