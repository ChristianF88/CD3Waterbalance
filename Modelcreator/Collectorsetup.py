...
class Collectorsetup:
    ...
    def __init__(self, numberofCollectors, starting_value_i, **Collectorattributes ):
        ...
        for i in range(numberofCollectors+starting_value_i)[starting_value_i:]:
            exec 'self.Collectorattributelist.append({"Collector_'+str(i)+'" : Collectorattributes})'
    ...
    def Setandwrite_attributes(self,numberofCollectors, starting_value_i, attributevector):
        for i in range(numberofCollectors+starting_value_i)[starting_value_i:]:
            self.Collectorattributelist[i][str('Collector_'+str(i))]["Number_of_Inports"] = attributevector[i][0]
    
        for i in range(numberofCollectors+starting_value_i)[starting_value_i:]:
            exec '''self.line1='\\t\\t\\t<node id="Collector_'+str(i)+'" class="Collector"/> \\n' '''
            exec '''self.line2='\\t\\t\\t\\t<parameter name="Number_of_Inports" type="int" value="'+str(self.Collectorattributelist[i][str('Collector_'+str(i))]["Number_of_Inports"])+'"/> \\n ' '''
